...
void BitBltToD3D11SubResource(BitMapData& pBitMapData);
void BitBltToFile(BitMapData& MyBitMapData);
...
/// <summary>
/// Erstellt einen VertexBuffer-Speicher. Mit CreateBuffer wird es auf GPU kopiert und mit SetVertexBuffers wird es von GPU auf GPU-"Ausführungs-VRAM" kopiert
/// </summary>
...
{
	HRESULT hResult = NULL;									// Rückgabewert von ApiFkt. für Errorhandling
	IBufContent.IndexBufferDesc.BindFlags
...
void CreateConstantBuffer(ConstantBufferContent& CBufConstant)
{
	...
	ConstantBufferDesc.ByteWidth = sizeof(CBufConstant.MyConstantBuffer);	// Gesamte Größe des Buffers: 4x4xfloat = 4*4*4 = 64 Bytes
	ConstantBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;				// wird pro Frame geändert, muss für CPU gelockt werden
	ConstantBufferDesc.MiscFlags = 0;
	ConstantBufferDesc.StructureByteStride = 0;								// Größe pro Element, wir dnicht benötigt
...
/// Die UV-TextureCoord werden von InputAssamblerStage bis in den PixelshaderStage übtertragen
...
void CreateAndSetSampler()
{
	...
	hResult = pD3dDevice->CreateSamplerState(&samplerDesc, &samplerState);	// Sampler erstellen auf GPU
	pD3dContext->PSSetSamplers(0, 1, samplerState.GetAddressOf());			// Sampler setzen (COPY von GPU zu GPU)
}//END-FUNC

struct ImgProcessingData
{
public:
	int iDeltaX = 0;
	int iDeltaY = 0;
...
void CreateImg(ImgProcessingData& ImgData)
{
	int iDeltaX = ImgData.iDeltaX;											// Übergabe an lokale Variablen, wegen der Übersicht
	int iDeltaY = ImgData.iDeltaY;
	...
	for (int y = 0; y < ImgData.iDeltaY; ++y)
	{
		for (int x = 0; x < ImgData.iDeltaX; ++x)
		{
			// y = 0;x = 0;
			...
			if (x <= 1000)
			{
				ImgData.pRGBQUAD->rgbRed = pData[y * iDeltaX * iBpp + x * iBpp] = 0xff; // red, 8 Bit
				ImgData.pRGBQUAD->rgbGreen = pData[y * iDeltaX * iBpp + x * iBpp + 1] = 0xff; // green, 8 Bit
				ImgData.pRGBQUAD->rgbBlue = pData[y * iDeltaX * iBpp + x * iBpp + 2] = 0x00; // blue, 8 Bit
				ImgData.pRGBQUAD->rgbReserved = pData[y * iDeltaX * iBpp + x * iBpp + 3] = 0xff; // alpha, 8 Bit
			}
			else
			{
				ImgData.pRGBQUAD->rgbRed = pData[y * iDeltaX * iBpp + x * iBpp] = 0xff; // red, 8 Bit
				ImgData.pRGBQUAD->rgbGreen = pData[y * iDeltaX * iBpp + x * iBpp + 1] = 0x00; // green, 8 Bit
				ImgData.pRGBQUAD->rgbBlue = pData[y * iDeltaX * iBpp + x * iBpp + 2] = 0x00; // blue, 8 Bit
				ImgData.pRGBQUAD->rgbReserved = pData[y * iDeltaX * iBpp + x * iBpp + 3] = 0xff; // alpha, 8 Bit
			}//END-IF
		}//END-FOR x
	...
	TextureDesc.Width = iDeltaX;							// Width
	TextureDesc.Height = iDeltaY;							// Height
	...
	TextureDesc.Usage = D3D11_USAGE_DEFAULT;				// Default: Read/Write für GPU
	...
	SubResData.SysMemPitch = iDeltaX * iBpp;				// Eine Pixelreihe als Width*4: 1 Pixel = 4 Bytes
	...
	pD3dDevice->CreateTexture2D(&TextureDesc, &SubResData, &ImgData.pTexture);
}//END-FUNC
/// <summary>
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	const UINT uiHeight = 1440;								// Auflösungsparameter
	const UINT uiWidth = 2560;
	...
	ClsWnd oMyWnd(											// führt CreateWindowEx aus, d.h. erzeugt Fenster die reg. sind. 
		...
		L"MyProgramm",										// Titelbar
		100,												// X-StartPosition	
		100,												// Y-StartPosition
		uiWidth,											// Breite des Fensters
		uiHeight);											// Höhe des Fensters
	...
	/**********ImageProcessing***********/
	ImgProcessingData ImgData;
	ImgData.iBpp = 4;
	ImgData.iDeltaX = uiWidth;
	ImgData.iDeltaY = uiHeight;
	CreateImg(ImgData);										// generiert ein zweifarbiges Img mit entsprechender Auflösung und Bittiefe (4Byte = 32Bit)
	/**********ShaderView****************/
	...
	hResult = pD3dDevice->CreateShaderResourceView(ImgData.pTexture.Get(), nullptr, &ShaderView);
	...
	delete ImgData.pData;
	ImgData.pData = nullptr;
	delete ImgData.pRGBQUAD;
	ImgData.pRGBQUAD = nullptr;
	return 0;
}//END-FUNC Main
...
/// </param>
void BitBltToD3D11SubResource(BitMapData& MyBitMapData)
{
	...
	strcpy_s(SrcWnd.title, MyBitMapData.strTitle);					// Titel des Fensters
	...
	hBitmap = CreateCompatibleBitmap(								// Hier werden nur Metadaten gesetzt, keine Pixeldaten kopiert
		...
		MyBitMapData.iBmpStretchWidth,								// aber mit größerer Auflösung
		MyBitMapData.iBmpStretchHeight
	);
	...
	StretchBlt(														// Kopieren der BitmapDaten die als Bitmap-Unterobj in den DCs existieren
		...
		MyBitMapData.iBmpStretchWidth,								// Breite ab X
		MyBitMapData.iBmpStretchHeight,								// Höhe ab Y
	...
	memcpy(MyBitMapData.pDataOFSubResource, pImgData, uiPixelDataSize); // Pixeldaten in D3D11 resource kopieren
...
void BitBltToFile(BitMapData& MyBitMapData)
{
	...
	strcpy_s(SrcWnd.title, MyBitMapData.strTitle);					// gesuchten Titel kopieren
	EnumWindows(EnumWindowsProc, reinterpret_cast<LPARAM>(&SrcWnd));// WindowHandle mit SrcWnd.title finden
	...
	ulFileSize =
		...
		+ (sizeof(RGBQUAD)											// ExtraBytes für Dateiende (Größe eines Pixels)
			+ (MyBitMapData.iBmpStretchWidth * MyBitMapData.iBmpStretchHeight * 4) // Größe der Pixeldaten
			);
	...
	pBmpHeader->biHeight = MyBitMapData.iBmpStretchHeight;			// Höhe
	pBmpHeader->biWidth = MyBitMapData.iBmpStretchWidth;			// Breite
	...
	GetDIBits(														// Bitweises kopieren
		...
		MyBitMapData.iBmpStretchHeight,								// Zeilen		
		pPixelData,													// Buffer, wo Pixeldaten reinkopiert werden. Ist PixelBuffer meines BmpFiles
		(LPBITMAPINFO)pBmpHeader,									// BmpHeader
		DIB_RGB_COLORS);
	hFile = CreateFileA(MyBitMapData.strBmpFileName, GENERIC_WRITE, FILE_SHARE_WRITE, 0, CREATE_ALWAYS, 0, 0); // File Blub.bmp erstellen
